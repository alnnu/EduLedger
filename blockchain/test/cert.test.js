...
contract("Cert", () => {
  ...
  it('should retrieve all certs', async () => {

    const allCerts = await certInstance.getAllCerts();

    assert.ok(allCerts.length > 0, "No certificates found");
  })

  it('should get a cert', async () => {

    const retrievedCert = await certInstance.getCert("metadataHashTest");
    ...
    assert.equal(retrievedCert.displayInfo.imageHash, "imageHashTest", "Image hash does not match");
    assert.equal(retrievedCert.displayInfo.metadataHash, "metadataHashTest", "Metadata hash does not match");
    assert.equal(retrievedCert.displayInfo.NFTid, 0, "NFT ID does not match");
  })
})
